...
export const authRouter = router({
    register: publicProcedure.input(UserRegistrationSchema).mutation(async (req) => {
        const { ctx, input } = req;

        // @@Todo: validate the `reCaptchaToken` in the input.
        if (ENV === "production") {
            throw new TRPCError({ code: "UNAUTHORIZED", message: "reCaptchaToken is required" });
        }
    ...
    login: publicProcedure.input(UserLoginSchema).mutation(async (req) => {
        ...
        return {
            ...await ctx.authService.createTokens({ id: credentials.id, name: credentials.name, email: credentials.email })
        }
    ...
    refresh: authProcedure.mutation(async (req) => {
        ...
        return await ctx.authService.refreshTokens(req.ctx.rawTokens.refreshToken);
    }),
});
