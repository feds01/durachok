...
import { ByPinRequestSchema, GameCreateRequestSchema, GameJoinRequestSchema, Player } from "../schemas/lobby";
    ...
    join: publicProcedure.input(GameJoinRequestSchema).mutation(async (req) => {
        ...
        const player: Player = expr(() => {
            if (ctx.token?.kind === 'registered') {
                return { name: input.name, socket: null, registered: ctx.token.user.id, confirmed: false }
            } else {
        ...
        if (isUserInLobby) {
            throw new TRPCError({ code: 'BAD_REQUEST', message: 'User already in lobby' });
        }
        ...
        return {
            ...(!player.registered && { ...await ctx.authService.createTokens({ pin: input.pin, name: input.name }) })
        }
    }),
});
